import { Avatar, AvatarPicker } from '../components/Common';
  ...
  return (
    <section className="page-view profile-view">
      ...
      <div className="profile-grid">
        ...
        {isOwn && <form className="panel-card profile-form" onSubmit={onSave}><p className="mini-label">Editar mi perfil</p><label>Username<input value={form.username || ''} onChange={(event) => setForm({ ...form, username: event.target.value })} placeholder="@tu_username" /></label><label>Nombre<input value={form.name || ''} onChange={(event) => setForm({ ...form, name: event.target.value })} /></label><label>Biografía<textarea value={form.bio || ''} onChange={(event) => setForm({ ...form, bio: event.target.value })} rows="4" /></label><button className="primary-btn">Guardar perfil</button></form>}
      </div>
